...
# ----------
#  Script de NiPeGun para preparar un Debian recién instalado para que pueda
...
UsuarioPPPSimyo="xxx"
ClavePPPSimyo="xxx"
MacDelRouterSimyo="00:00:00:00:00:00"
...
if [ $OS_VERS == "7" ]; then
  ...
  echo ""
  echo "------------------------------------------------------------------------------------------"
  echo "  Iniciando el script de preparación para conectar 10 (Buster) con la ONT de Simyo..."
  echo "------------------------------------------------------------------------------------------"
  echo ""
  ...
  echo "  pre-up iptables-restore < /root/ReglasIPTablesIP4RouterSimyo.ipt" >> /etc/network/interfaces
  ...
  echo "  #hwaddress ether $MacDelRouterSimyo # Necesario para evitar futuros problemas" >> /etc/network/interfaces
  echo ""                                                                                >> /etc/network/interfaces
  ...
  echo ""
  echo "noipdefault"                 > /etc/ppp/peers/SimyoWAN
  echo "defaultroute"               >> /etc/ppp/peers/SimyoWAN
  echo "replacedefaultroute"        >> /etc/ppp/peers/SimyoWAN
  echo "hide-password"              >> /etc/ppp/peers/SimyoWAN
  echo "#lcp-echo-interval 30"      >> /etc/ppp/peers/SimyoWAN
  echo "#lcp-echo-failure 4"        >> /etc/ppp/peers/SimyoWAN
  echo "noauth"                     >> /etc/ppp/peers/SimyoWAN
  echo "persist"                    >> /etc/ppp/peers/SimyoWAN
  echo "#mtu 1492"                  >> /etc/ppp/peers/SimyoWAN
  ...
  echo ""
  echo '"'$UsuarioPPPSimyo'" * "'$ClavePPPSimyo'"' > /etc/ppp/chap-secrets

  echo ""
  echo -e "${ColorVerde}Agregando datos al archivo pap-secrets...${FinColor}"
  echo ""
  echo '"'$UsuarioPPPSimyo'" * "'$ClavePPPSimyo'"' >> /etc/ppp/pap-secrets

  echo ""
  echo -e "${ColorVerde}Creando las reglas de IPTables...${FinColor}"
  echo ""
  echo "*mangle"                                                                                  > /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":PREROUTING ACCEPT [0:0]"                                                                >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":INPUT ACCEPT [0:0]"                                                                     >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":FORWARD ACCEPT [0:0]"                                                                   >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":OUTPUT ACCEPT [0:0]"                                                                    >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":POSTROUTING ACCEPT [0:0]"                                                               >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "COMMIT"                                                                                  >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ""                                                                                        >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "*nat"                                                                                    >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":PREROUTING ACCEPT [0:0]"                                                                >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":INPUT ACCEPT [0:0]"                                                                     >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":OUTPUT ACCEPT [0:0]"                                                                    >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":POSTROUTING ACCEPT [0:0]"                                                               >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "-A POSTROUTING -o ppp0 -j MASQUERADE"                                                    >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "COMMIT"                                                                                  >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ""                                                                                        >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "*filter"                                                                                 >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":INPUT ACCEPT [0:0]"                                                                     >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":FORWARD ACCEPT [0:0]"                                                                   >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo ":OUTPUT ACCEPT [0:0]"                                                                    >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "-A FORWARD -i ppp0 -o $InterfazCableada2 -m state --state RELATED,ESTABLISHED -j ACCEPT" >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "-A FORWARD -i $InterfazCableada2 -o ppp0 -j ACCEPT"                                      >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  echo "COMMIT"                                                                                  >> /root/ReglasIPTablesIP4RouterSimyo.ipt
  ...
  echo ""
  echo "pon SimyoWAN" >> /root/scripts/ComandosPostArranque.sh

  echo ""
  echo -e "${ColorVerde}--------------------------------------------------------------------------------------------${FinColor}"
  echo -e "${ColorVerde}Ejecución del script de preparación para la fibra de Simyo, finalizada.${FinColor}"
  echo ""
  echo -e "${ColorVerde}Apagando el dispositivo...${FinColor}"
  echo ""
  echo -e "${ColorVerde}Al acabar de apagarse ya podrás conectar un cable desde el puerto eth0 a la ONT de Simyo ${FinColor}"
  ...
  echo ""
  echo "--------------------------------------------------------------------------------------------"
  echo "  Iniciando el script de preparación para conectar 11 (Bullseye) con la ONT de Vodafone..."
  echo "--------------------------------------------------------------------------------------------"
  echo ""
